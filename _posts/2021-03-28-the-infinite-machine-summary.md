...
The obvious option would be to build it on top of Bitcoin, as Mastercoin was doing, but Vitalik discarded that idea as the Bitcoin community was mired in a heated discussion over one of the protocol’s features. Developers were getting ready to release an update to the cryptocurrency’s protocol that changed the OP\_RETURN script, which allowed arbitrary data to be stored in the Bitcoin blockchain. The March 2014 update cut the amount stored in each transaction to 40 bytes from 80 bytes 3 . 
...
Please support the author by buying the book if you like it.
